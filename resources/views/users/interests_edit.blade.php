...
@endsection
@section('content')
    <form action="{{route('users.interests.update',['user'=>$user->id])}}" method="POST">
        {{csrf_field()}}
                ...
                @foreach($interests as $interest)
                    <!-- Primary Multi-Select Tiles -->
                        <div class="primary-gateway-tile">
                            <input {{$user->interests->contains($interest)? 'checked': ''}} name="interests[]" type="checkbox" value="{{$interest->id}}" id="{{$interest->id}}"/>
