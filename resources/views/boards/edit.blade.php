...
@section('content')
    <div class="card stick-create-card offset-3">
        ...
        <form action="{{route($pageUrl.'.boards.update',['record'=>$record, 'board'=>$board])}}" method="POST" enctype="multipart/form-data">
            {{csrf_field()}}
                ...
                <label for="city_id" class="control-label">Ilgi alanları</label>
                    ...
                    <select class="js-example-tags1" multiple="multiple" style="width: 100%" required name="interests[]" id="interests"
                        ...
                        @foreach($interests as $interest)
                            <option {{$board->interests->contains($interest)? 'selected': ''}} value="{{ $interest->id }}">{{ $interest->name }}</option>
                        @endforeach
                    </select>
